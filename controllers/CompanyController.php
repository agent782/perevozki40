    ...
    public function behaviors()
    {
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'actions'=>['index', 'create', 'validate-add-company'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
...
//   //                 return 'Add company to Profile';
        ...
        if($modelProfile->getMaxCompanies() <= $modelProfile->getCountCompanies()){
            Yii::$app->session->setFlash('maxCompanies', FAQ::getFAQ(Yii::$app->user->getId(), FAQ::FAQ_MAX_COMPANIES));
            return $this->redirect('index');
        }
    ...
    public function actionUpdate($id)
    {
        ...
        $XcompanyXprofile = $modelCompany->xprofileXcompany;
        if ($modelCompany->load(Yii::$app->request->post()) && $modelCompany->save()) {
            return $this->redirect(['view', 'id' => $modelCompany->id, 'XcompanyXprofile' => $XcompanyXprofile,]);
        } else {
