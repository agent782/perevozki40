...
//    const PAID_CANCELED = 2;
    ...
    public function scenarios()
    {
        ...
        $scenarios[self::SCENARIO_UPDATE_STATUS] = ['status', 'update_at', 'valid_datetime'];
...
//       $res[self::STATUS_DISPUTE] = 'Открыт спор';
    ...
    public function getShortRoute(){
        $route = $this->route;
        $return = '(&asymp;' . $route->distance . 'km)* '. $route->startCity . ' -';
        for($i = 1; $i<9; $i++){
            $attribute = 'route' . $i;
...
//                    return $this->update_at . ' ' . $this->create_at;
                    $this->getErrors();
                    $this->save();
                    $this->setEventChangeStatusToExpired();
                    break;
